...
class AccountifyClient:
    ...
    async def create_account(
        ...
        *,
        ...
        """Create a new account on the chart of accounts.

        `account_type` must be one of: asset, liability, equity, revenue, expense.
        `code` is auto-generated by the backend if omitted.
        """
        payload: dict[str, Any] = {
            "name": name,
        ...
        r = await self._request("POST", "/api/accounts/", json=payload)
        if r.status_code not in (200, 201):
            raise AccountifyError(f"create_account: {r.status_code} {r.text}")
        return r.json()
